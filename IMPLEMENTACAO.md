# 📋 Resumo de Implementação - Exercícios de Sockets (TCP + UDP)
...
### ✅ Pacote `br.com.projeto.udp`

**Responsabilidade**: Implementar um chat simples via UDP em modo **P2P**.

**Recursos Implementados:**
- ✅ Dois usuários trocam mensagens via UDP
- ✅ Cada instância abre um `DatagramSocket` na porta local (lado “servidor”)
- ✅ Thread dedicada para receber (`receive()`)
- ✅ Thread principal envia mensagens digitadas no teclado (`send()`)
- ✅ Cada mensagem recebida mostra **quem mandou** (nome + IP:porta) e o texto
- ✅ Tratamento de entradas inválidas e erros de rede

### ✅ Classe ChatUdpPeer.java
**Responsabilidade**: Aplicação executável do chat UDP.

**Parâmetros de execução:**
```
<portaLocal> <hostRemoto> <portaRemota> [nome]
```

**Fluxo do Peer:**
```
[Abrir DatagramSocket] → [Iniciar Thread Recebedora] → 
[Ler Teclado] → [Enviar DatagramPacket] → [Receber DatagramPacket] → ...
```

### ✅ Classe ChatMensagem.java
**Responsabilidade**: Modelo de dados de mensagem do chat.

Armazena:
- Nome do remetente
- Texto

### ✅ Classe ChatMensagemCodec.java
**Responsabilidade**: Converter mensagem ↔ bytes (UDP payload).
...
Observação: apenas o **primeiro** `|` separa nome e texto (o texto pode conter `|`).
...
APIs adicionais do Exercício 2 (UDP):
```java
...
### Exercício 2 (UDP)
- ✅ **Chat usando UDP**: troca de mensagens via `DatagramSocket`
- ✅ **Dois usuários**: duas instâncias do peer trocam mensagens
- ✅ **Indica remetente**: nome + IP:porta em cada mensagem recebida
- ✅ **P2P**: cada lado é cliente e servidor UDP ao mesmo tempo
...
### Exercício 2 - Chat UDP (2 terminais)
