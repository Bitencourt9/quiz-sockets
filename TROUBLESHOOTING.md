# 🛠️ Guia de Troubleshooting - Exercícios de Sockets (TCP + UDP)
...
## 💬 Exercício 2 (UDP) - Problemas Comuns

### Problema 11: "Port already in use" (porta UDP ocupada)

**Causa**: A porta local escolhida (ex.: 6000) já está em uso por outro processo.

**Solução**:
1. Tente outra porta (ex.: 6002 e 6003)
2. Verifique quem está usando a porta:
   ```bash
   netstat -ano | findstr :6000
   ```
3. (Opcional) Finalize o PID que estiver usando a porta:
   ```bash
   taskkill /PID <PID> /F
   ```
...
### Problema 12: "Mensagens não chegam" (UDP)

**Causas comuns**:
- Host remoto errado (ex.: `localhost` quando deveria ser IP da outra máquina)
- Porta remota errada
- Firewall bloqueando UDP

**Solução**:
1. Confirme os comandos (host/porta cruzados):
   - Terminal A: portaLocal=6000 → remoto=6001
   - Terminal B: portaLocal=6001 → remoto=6000
2. Teste primeiro em localhost (mesma máquina)
3. Se for entre máquinas, libere a porta UDP no firewall

---

### Problema 13: "Host remoto inválido" (UnknownHostException)

**Causa**: O hostname passado não foi resolvido.

**Solução**:
1. Use `localhost` para testes locais
2. Ou use um IP direto (ex.: `192.168.0.10`)

---

### Problema 14: "Recebe, mas não mostra nome" (remetente)

**Causa**: O peer remoto pode estar enviando payload sem o formato `nome|texto`.

**Solução**:
- Use a mesma classe `ChatUdpPeer` nos dois lados (ela envia `nome|texto` automaticamente)
